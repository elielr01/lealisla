 ...
 *
 * $Header: /cvs/bbclone/lib/referrer.php,v 1.35 2005/04/03 20:26:38 olliver Exp $
 *
...
function bbc_get_search($array, $char) {
  ...
  foreach ($array as $string) {
    ...
    foreach ($query as $key) {
      ...
      if (($char ? mb_strlen($term) : strlen($term)) < 2) {
        $matches = array();
        continue;
      }
...
function bbc_get_keywords($ref, $char) {
  ...
  $raw_search = bbc_get_search($get_vars, $char);
